...
export default function Sidebar({ sessions, activeId, onNew, onLoad, onDelete }) {
  ...
  return (
    <aside className="w-64 min-w-[256px] flex flex-col h-screen border-r border-slate-700/50 relative z-10"
    ...
    >
        ...
        <AnimatePresence initial={false}>
          {sorted.length === 0 ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              className="text-center text-[12px] text-slate-600 py-8 px-4 leading-relaxed"
            >
            ...
            <motion.div key={id} layout
              ...
              onClick={() => onLoad(id)}
              className={`group flex items-center gap-2.5 px-3 py-2.5 rounded-xl cursor-pointer mb-0.5 transition-all ${
                id === activeId
            ...
            >
              ...
              <motion.button
                ...
                className="opacity-0 group-hover:opacity-100 p-1 rounded-md hover:bg-red-500/15 hover:text-red-400 text-slate-600 transition-all"
                whileTap={{ scale: 0.9 }}
              >
                <Trash2 size={11} />
              </motion.button>
      ...
      <div className="px-4 py-3 border-t border-slate-700/50 flex items-center gap-2">
        ...
        <span className="text-[11px] text-slate-500">
          <span className="text-indigo-400 font-medium">Groq</span> · Llama 3.3 70B
        </span>
      </div>
    </aside>
  )
