      ...
      <div className="flex items-center justify-between px-4 py-2 bg-slate-900/80 border-b border-slate-700/60">
        ...
        <button onClick={copy}
          className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-medium border transition-all ${
            copied ? 'text-emerald-400 border-emerald-500/30 bg-emerald-500/8' : 'text-slate-500 border-slate-700 hover:text-slate-300 hover:bg-slate-700/50'
          }`}
        >
...
export function StreamingMessage({ fullText, onDone }) {
  ...
  return (
    <div className="text-sm text-slate-200 leading-relaxed whitespace-pre-wrap">
      {displayed}
      <span className="inline-block w-[2px] h-[14px] bg-indigo-400 ml-0.5 align-text-bottom animate-blink" />
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      <div className="relative flex-shrink-0">
        ...
        <div className={`relative w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold z-10 ${
          isUser
      ...
      <div className={`flex flex-col gap-1 max-w-[76%] ${isUser ? 'items-end' : 'items-start'}`}>
        <div className={`px-4 py-3 rounded-2xl text-sm leading-relaxed ${
          isUser
            ? 'bg-gradient-to-br from-indigo-600 to-violet-600 text-white rounded-tr-sm shadow-lg shadow-indigo-600/25'
            : 'bg-slate-800 border border-slate-700/60 text-slate-200 rounded-tl-sm'
        }`}>
          {isUser ? (
            <p className="whitespace-pre-wrap leading-relaxed">{content}</p>
          ) : isStreaming && streamText ? (
            <StreamingMessage fullText={streamText} onDone={onStreamDone} />
          ) : (
