  ...
  return (
    ...
    >
      <div className="relative flex-shrink-0">
        ...
        <div className="relative w-8 h-8 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-xs font-bold text-white z-10 shadow-lg">
          AI
        </div>
      </div>
      <div className="px-4 py-3 rounded-2xl rounded-tl-sm bg-slate-800 border border-slate-700/60 flex items-center gap-2">
        ...
        {[0, 1, 2].map(i => (
          <motion.span key={i}
            className="w-2 h-2 rounded-full bg-gradient-to-br from-indigo-400 to-violet-500"
...
function Welcome({ onChipClick }) {
  return (
    ...
    >
      <motion.div
        className="w-16 h-16 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center mb-5 animate-float"
      ...
      <p className="text-[14px] text-slate-400 max-w-md leading-relaxed mb-8">
        Expert help for Apex, SOQL, LWC, Flows, integrations, and everything Salesforce.
      </p>
...
export default function ChatArea({ history, isLoading, streamText, onStreamComplete, onChipClick }) {
  ...
  // Reset stream state when new stream starts
  useEffect(() => { setStreamDone(false) }, [streamText])

  // Auto-scroll on new content
  useEffect(() => {
    bottomRef.current?.scrollIntoView({ behavior: 'smooth' })
  }, [history.length, isLoading, streamText])

  // When streaming finishes, notify App to clear streamText
  // so history renders the final message naturally
  const handleStreamDone = () => {
    setStreamDone(true)
    setTimeout(() => onStreamComplete?.(), 100)
  }

  // While streaming: hide the last assistant message (we're rendering it via typewriter)
  // After streaming done: streamText is cleared by App, full history renders
        ...
        <div className="max-w-[800px] mx-auto px-6 py-8 flex flex-col gap-5">
          <AnimatePresence initial={false}>
            ...
            {streamText && !streamDone && (
              <Message key="stream" role="assistant" content="" isStreaming streamText={streamText} onStreamDone={handleStreamDone} />
            )}
