...
const load = () => { try { return JSON.parse(localStorage.getItem('sf_sessions') || '{}') } catch { return {} } }
...
export default function App() {
  ...
  const save = useCallback((s) => {
    setSessions(s)
    localStorage.setItem('sf_sessions', JSON.stringify(s))
  }, [])
  ...
  const sendMessage = useCallback(async (msg) => {
    ...
    try {
      ...
      // Save full history first, then trigger streaming
      save({
        ...currentSessions,
        [sessId]: { ...currentSessions[sessId], history: data.history }
      })
  ...
  // Called by ChatArea when typewriter finishes — clears stream so history renders naturally
