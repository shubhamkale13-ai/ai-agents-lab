...
function formatBody(body) {
  return body.split('\n').map((line, index) => {
    ...
    if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
      return (
        <li key={`item-${index}`} className="ml-5 list-disc leading-7">
          {trimmed.slice(2)}
        </li>
      )
    }
...
export default function AIResponseCard({ content, metadata, isDark, onRegenerate }) {
  ...
  return (
    <div
      className={`w-full rounded-[30px] border p-5 shadow-2xl ${
        isDark
          ? 'border-white/10 bg-slate-950/55 shadow-indigo-950/20'
          ...
          return (
            <div
              ...
              className={`rounded-3xl border ${
                isDark ? 'border-white/8 bg-white/[0.03]' : 'border-slate-200 bg-slate-50/80'
              }`}
            >
              <button
                type="button"
              ...
              {!isCollapsed && (
                <div className={`border-t px-4 py-4 text-sm ${isDark ? 'border-white/8 text-slate-200' : 'border-slate-200 text-slate-700'}`}>
